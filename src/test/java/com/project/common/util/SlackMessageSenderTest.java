...
class SlackMessageSenderTest {
  ...
  void sendMessageSuccess() throws Exception {
    ...
    when(methods.chatPostMessage(any(ChatPostMessageRequest.class))).thenReturn(response);
  ...
  void sendMessageFail() throws Exception {
    ...
    when(slack.methods("TEST_TOKEN")).thenReturn(methods);
    when(methods.chatPostMessage(any(ChatPostMessageRequest.class))).thenReturn(failResponse);
