...
public class RankingDayRepositoryTest {
    @Autowired
    RankingDayRepository rankingDayRepository;
    @Autowired
    EntityManager em;
    ...
    private UserEntity createUser(String name) {
        UserEntity u = UserEntity.builder()
                .slackId(name).baekjoonId(name).username(name).totalSolvedCount(0).build();
        em.persist(u);
        return u;
    }
    ...
    private ProblemEntity createProblem(int level) {
        ProblemEntity p = ProblemEntity.builder()
                .problemId(1000 + level).problemTitle("Level " + level)
                .problemLevel(level).problemUrl("url").build();
        em.persist(p);
        return p;
    }
