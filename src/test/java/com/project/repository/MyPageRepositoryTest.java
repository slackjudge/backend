...
class MyPageRepositoryTest {
    ...
    void setUp() {
        ...
        createSolvedHistory(user, p2, LocalDate.of(2025, 12, 5).atTime(14,0));
    ...
    void findGrassListTest() {
        ...
        //when
        List<MyPageResponse.Grass> result = myPageRepository.findGrassList(user.getUserId(), year, month);
    ...
    private ProblemEntity createProblem(int id, String title, int level) {
        ProblemEntity p = ProblemEntity.builder()
                .problemId(id).problemTitle(title).problemLevel(level).problemUrl("url").build();
        em.persist(p);
        return p;
    }
    private void createSolvedHistory(UserEntity u, ProblemEntity p, LocalDateTime time) {
        em.persist(UsersProblemEntity.builder()
                .user(u).problem(p).isSolved(true).solvedTime(time).build());
    }
