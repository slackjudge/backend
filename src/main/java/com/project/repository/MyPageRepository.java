...
public class MyPageRepository {
    ...
    /**
     * 월간 잔디 데이터 조회(날짜별 풀이 수 집계)
     * TO_CHAR함수를 사용해 날짜를 포맷팅하고 그룹핑합니다.
     *
    ...
    public List<MyPageResponse.Grass> findGrassList(Long userId, int year, int month) {
        ...
        StringExpression dateStr = Expressions.stringTemplate(
                "TO_CHAR({0}, 'YYYY-MM-DD')",
                usersProblemEntity.solvedTime
        ).stringValue();

        return queryFactory
                .select(Projections.constructor(MyPageResponse.Grass.class,
                        dateStr,
    ...
    public List<MyPageResponse.Problem> findSolvedProblemsByDate(Long userId, LocalDate date) {
        ...
        return queryFactory
                .select(Projections.constructor(MyPageResponse.Problem.class,
